from sympy import Matrix, pprint, Integer, floor
import itertools
...
class BasisElement(LiftableVector):
	...
	def __le__(self, other):
		return all([u_i <= v_i for u_i, v_i in itertools.izip(self[:-1], other[:-1])]) and \
			abs(self[-1]) <= abs(other[-1]) and \
			self[-1]*other[-1] >= 0
	...
	def compute_alpha(self, vector_g):
		return min([floor(s_i/g_i) for s_i, g_i in zip(self[:], vector_g[:]) if g_i != 0])
...
class ExtremeRay(LiftableVector):
	...
	def compute_alpha(self, vector_g):
		...
		pprint(self_copy)
		pprint(g_copy)
		return min([s_i/g_i for s_i, g_i in zip(self_copy[:], g_copy[:]) if g_i != 0])
